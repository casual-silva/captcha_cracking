...
def main():
    ...
    driver.get("https://admin.xiaoe-tech.com/login_page?xeuti=ituex#/acount")

    try:
        # 获取手机号码输入框
        text_box = wait.until(expected_conditions.presence_of_element_located((By.XPATH, '//div[@class="inputBox"]/input')))
        text_pwd = wait.until(expected_conditions.presence_of_element_located((By.XPATH, '//div[@class="passwordWrapper"]/div/input[@type="password"]')))
        # 获取发送验证按钮
        send_btn = wait.until(expected_conditions.presence_of_element_located((By.XPATH, '//div[@data-sensors="登录_登录页_点击登录"]')))
        print(send_btn)
    ...
    text_box.send_keys("15889391177")
    text_pwd.send_keys('elecfans@2018')
    ...
    try:
        ...
        # 获取 滑块
        slider = wait.until(
            expected_conditions.presence_of_element_located((By.XPATH, '//div/img[@id="slideBlock"]')))
